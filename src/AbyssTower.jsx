...
const SKILL_CAP = 5; // 1ランで習得できるスキル数の上限(全8種は覚えられない)
...
const ABILITY_MAP = Object.fromEntries(ABILITIES.map(a => [a.key, a]));
// レアリティ別の固有能力付与率(コモン→神器)
const ABILITY_CHANCE = [0.06, 0.14, 0.24, 0.38, 0.55, 0.75];
...
const SLOT_KEYS = ["weapon", "armor", "helmet", "boots", "ring", "amulet"];
const PREFIXES = ["", "", "鋭利な", "重厚な", "輝く", "嵐の", "深淵の", "王の"];
...
const CURSE_CHANCE = 0.16; // レア以上のアイテムがこの確率で呪いを帯びる
const CURSE_BOOST = 1.45;  // 呪い装備は素の数値がこの倍率で強化される
...
const ELITE_TRAIT_KEYS = Object.keys(ELITE_TRAITS);
...
const hasNode = (player, key) => (player.tree || []).includes(key);
const RELIC_CAP = 6; // レリックの同時所持上限(超える場合は入れ替えが必要)
...
const hasRelic = (player, flag) => (player.relics || []).some(k => RELIC_MAP[k]?.flag === flag);
...
const DIFF_RAMP_FLOORS = 5; // 難易度倍率がこの階数までかけて本来の値まで上がる(各難易度のrampFloorsが優先。フォールバック用)
...
const FINAL_PATTERN = ["roar", "flurry", "heavy", "guard", "heavy"]; // 深淵の魔王
...
const STAT_LABELS = { atk: "攻撃", def: "防御", hp: "HP", crit: "クリ率", critDmg: "クリ倍率", lifesteal: "吸血", double: "連撃", thorns: "棘", poisonPower: "毒威力", dmgVsStatus: "対状態異常", perHitHeal: "ヒット毎回復", afterDefendDmg: "防御後火力", onKillHeal: "撃破時回復", critVsCC: "対行動不能クリ確定", defendThornsMult: "防御中棘倍加", onDefendFreezeCh: "防御時凍結", onCritCd: "クリ時CD短縮", betterDefend: "防御強化(-80%)", gambleDmg: "気まぐれな打撃", alwaysPoison: "攻撃毎に毒", thornsScale: "棘に攻撃力加算", thornsDef: "棘に防御力加算", thornsPoison: "棘が毒を付与", thornsCrit: "棘クリティカル率" };
const PCT_KEYS = ["crit", "critDmg", "lifesteal", "double", "poisonPower", "dmgVsStatus", "afterDefendDmg", "onKillHeal", "onDefendFreezeCh"];
const LOG_COLORS = { info: "#a8a29e", dmg: "#f87171", hurt: "#fb923c", heal: "#4ade80", gold: "#fbbf24" };
      ...
      {!(stats.noSkill > 0) && (
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          {player.skills.map(k => {
            ...
            return (
              <button key={k} onClick={() => useSkill(k)} disabled={dis} style={{ ...btnStyle(dis, "#7c2d12"), fontSize: 13 }}>
